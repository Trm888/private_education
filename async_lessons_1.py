...
# На высоком уровне используем только asyncio.run
async def foo1():
    await asyncio.sleep(4)
    print("Hello from foo1! (1)")
    await asyncio.sleep(3)
    return "Hello from foo1! (2)"


async def foo2():
    await asyncio.sleep(1)
    print("Hello from foo2! (1)")
    await asyncio.sleep(1)
    return "Hello from foo2! (2)"


async def main():
    results = await asyncio.gather(foo1(), foo2())
